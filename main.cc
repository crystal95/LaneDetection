...
#define line_width 4
#define template_bord 3
#define line_height 2
#define cross_corr_th 0.7//0.85
...
Mat input_img(raw_mat2.rows, raw_mat2.cols, CV_8UC1); 

      cvtColor(raw_mat2,input_img, CV_BGR2GRAY) ;
      //  Mat input_image = channelImage;
 //imshow( "my thresholded image", input_image); 
// sleep(1);
Mat thr_img(input_img.rows, input_img.cols, CV_8UC1); 
    Mat cross_corr_template = Mat::zeros( line_height, line_width+2*template_bord, CV_8UC1);
    for(int i=0;i<line_height;i++){
    for(int j=template_bord;j<line_width+template_bord;j++){
      cross_corr_template.at<uchar>(i, j) = 255;
    }
  }
  //matchTemplate( input_img, cross_corr_template, thr_img, CV_TM_CCORR_NORMED ); //converts output type to CV_8FC1 (0-1)
    ...
    morphologyEx( input_img, thr_img, MORPH_TOPHAT, element, Point(-1,-1), 10 );   
    threshold( thr_img, thr_img, 50, 255,0 ); //converts output type to CV_8FC1 (0-255)
  
    imshow( "my thresholded image", thr_img); 
...
int main(int argc, char **argv)
{
  ...
  if(initConfiguration()<0)
  {
   ...
   for(int seqid=0;seqid<=1200;seqid++)
  {     
    std::ostringstream ss;
    ss << std::setw(6) << std::setfill('0') << seqid;
    std::string s2(ss.str());
    string filename_in = "/home/danish/Desktop/Dataset1/Kitti/21/image_0/"+s2+".png";
