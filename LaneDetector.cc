 ...
 */
 ...
 void lane_tracking(Mat input,Mat *output, vector<Spline> &subimageSplines,LaneDetectorConf *lineConf, 
                         IPMInfo &ipmInfo, CameraInfo &cameraInfo)
{ 
  vector<Spline> extended_spline;
    track_cnt++;
    cout<<"track cnt is :"<<track_cnt<<endl;
    if(track_cnt==1)
   ...
   */
    ...
    cv::Mat final_tmp = cv::Mat::zeros(input.rows,input.cols, CV_8UC1);
    cv::Mat final_color;
    CvMat temp_in = input;
    CvMat temp_in2 =input;
    ...
    /*     track.at<uchar>(x0,y0)=0;
         ...
         for(i=0;i<subimageSplines.size();i++)
        {
            
          cout<<"here i begin"<<i<<endl;
          CvMat *pixels1 = mcvGetBezierSplinePixels(subimageSplines[i],.05, cvSize(temp_in.width, temp_in.height),true);
          cout<<"initial points"<<pixels1->height<<endl;
           for (int i=0; i<pixels1->height; i++)
          {
             int x5=(int)cvGetReal2D(pixels1, i, 0); 
              int y5=(int)cvGetReal2D(pixels1, i, 1);
            // cout<<"initialx "<<x5<<"  "<<"initialy "<<y5<<endl; 
          }

        CvMat *points = mcvEvalBezierSpline(subimageSplines[i], .1); //.05
       
            /////////////////////////////////////////
cout<<"linePixelsTangent danish "<< lineConf->extendIPMLinePixelsTangent<<endl;
        CvMat* p = mcvExtendPoints(&temp_in, points,
        ...
        /////////////////////////////////////////////////////
        /////////////////////////////////////////////
      //SHOW_IMAGE(&temp_in, "dont know anything", 1);

extended_spline.push_back(spline);
      for(unsigned int px=0;px<4;px++)
       {
      //cout<<cvPointFrom32f(spline.points[px]).x<<" "<<cvPointFrom32f(spline.points[px]).y<<endl;
      cvCircle(&temp_in, cvPoint(cvPointFrom32f(spline.points[px]).x,cvPointFrom32f(spline.points[px]).y) , 8, CV_RGB(255, 255, 255),-1 );
        }  /////////////////////////////////////////////////////////////////////

          CvMat *pixels = mcvGetBezierSplinePixels(spline,.05, cvSize(temp_in.width, temp_in.height),true);

          cout<<"final points"<<pixels->height<<endl;
          for (int i=0; i<pixels->height; i++)
          {
             int x4=(int)cvGetReal2D(pixels, i, 0); 
              int y4=(int)cvGetReal2D(pixels, i, 1);
            //  cout<<"finalx "<<x4<<"  "<<"finaly "<<y4<<endl; 
          }
            //////////////////////////////////////
         ...
         //   imshow("track", track);
            ...
            /*  cvLine(&temp_in, cvPoint((int)cvGetReal2D(pixels, i, 0),
                          (int)cvGetReal2D(pixels, i, 1)),
           cvPoint((int)cvGetReal2D(pixels, i+1, 0),
                               (int)cvGetReal2D(pixels, i+1, 1)),
           CV_RGB(255,250,250), 10);*/
             // mcvDrawSpline(&temp_in,subimageSplines[i], CV_RGB(255,255,255), 1);
              //SHOW_IMAGE(&temp_in, "Danish_Spline", 10);
          }
          cvReleaseMat(&pixels);
        }

      //    SHOW_IMAGE(&temp_in, "Danish_Spline", 10);
       //cvtColor(&temp_in,&temp_in, CV_BGR2GRAY)  ;
        cv::Mat tmp_out = cv::Mat(&temp_in);
        cout<<"mychannels are "<<" "<<tmp_out.channels();

   //sodhi  imshow( " track imge", track);  
  ...
  for( i = 0; i < result.rows; i++)
    {
      for(j = 0; j < result.cols; j++)
      {
          if(final.at<uchar>(i,j)>=200)
          final_tmp.at<uchar>(i,j)=255;
        else 
          final_tmp.at<uchar>(i,j)=0;
      }
    }
    *output=final_tmp;
   /* vector<CvRect> boxes_tmp;

 mcvGetSplinesBoundingBoxes(extended_spline,  LINE_HORIZONTAL,
                               cvSize(final_tmp.cols, final_tmp.rows),
                               boxes_tmp);


cv::cvtColor(final_tmp,final_color, CV_GRAY2BGR);

//boxes_tmp.size()
for(int l=0;l<=0;l++){
 rectangle(final_color,
           Point(boxes_tmp[l].y ,boxes_tmp[l].x),
           Point(boxes_tmp[l].y+boxes_tmp[l].height,boxes_tmp[l].x+boxes_tmp[l].width),
           Scalar( 0, 0, 255 ),
           1,
           8 );
}
 imshow( "my track results tmp", final_color);  */
  ...
  //sodhi imshow( "my frame result",  final_tmp );  
    save++;
   std::ostringstream ss;
    ss << std::setw(6) << std::setfill('0') << save;
    std::string s2(ss.str());
    string filename_in = "/home/danish/Desktop/results_trackin/test1/"+s2+".png";
  imwrite( filename_in, track );
  //sodhi     imshow( "my track results", final );  


 /*LineState* state_tmp;

 vector<Line> subimageLines_tmp;
    vector<Spline> subimageSplines_tmp;
    vector<float> subimageLineScores_tmp, subimageSplineScores_tmp;
 mcvGetLines(subimage, LINE_VERTICAL, subimageLines_tmp, subimageLineScores_tmp,
        subimageSplines_tmp, subimageSplineScores_tmp, stopLineConf,
        state_tmp); 
        */
} 
                 ...
                 LineState* state)
{
  ...
  //TODO: smooth image
  CvMat *image = cvCloneMat(inImage);
  CvMat *image_tmp = cvCloneMat(inImage);
...
//     #endif
  ...
  list<CvPoint>::iterator outPixelsi;
  mcvGetIPM(image, ipm, &ipmInfo, cameraInfo, &outPixels);
   SHOW_IMAGE(ipm, "This is my IPMimage", 1);
  //int wait=1;
//SHOW_IMAGE(ipm, "hey", wait);
...
#warning "Check: Smoothing IPM image"
  ...
  //unsigned char wx = 2;
  //unsigned char wy = 2;
  FLOAT sigmax = 1;//stopLinePixelWidth/4;
  FLOAT sigmay = 1;//stopLinePixelHeight;
...
#warning "clean negative parts in filtered image"
  mcvFilterLines(inImage, image_tmp, stopLineConf->kernelWidth,
                 stopLineConf->kernelHeight, sigmax, sigmay,
                 LINE_VERTICAL);
  /*cout<<"stopLineConf->kernelWidth"<<int(stopLineConf->kernelWidth)<<endl;
  cout<<"stopLineConf->kernelHeight"<<int(stopLineConf->kernelHeight)<<endl;
  cout<<"stopLineConf->kernelWidth"<<sigmax<<endl;
  cout<<"stopLineConf->kernelHeight"<<sigmay<<endl;*/
 // SHOW_IMAGE(ipm, "Lane unthresholded filtered");
  mcvThresholdLower(ipm, ipm, 0);

 // cv::Mat track = cv::Mat::zeros(inImage->width,input->height, CV_8UC1);
  //mcvThresholdLower(image_tmp,image_tmp, 0);
  //SHOW_IMAGE(image_tmp, "Lane thresholded filtered");
...
// 		    state);
    //put back
/*  vector<CvRect> boxes_tmp(state->ipmBoxes);
    cv::Mat tmp_sub=cv::Mat(subimage);
        cvtColor(tmp_sub,tmp_sub, CV_GRAY2BGR)  ;

for(int l=0;l<=boxes_tmp.size();l++){
 rectangle(tmp_sub,
           Point(boxes_tmp[l].x,boxes_tmp[l].y),
           Point(boxes_tmp[l].x+boxes_tmp[l].width,boxes_tmp[l].y+boxes_tmp[l].height),
           Scalar( 0, 0, 255 ),
           1,
           8 );
}
 imshow( "subimage show", tmp_sub);  */
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    cv::Mat track_tmp = cv::Mat(subimage); 
    cv::Mat output=cv::Mat(subimage);
    lane_tracking(track_tmp,&output,subimageSplines,stopLineConf, ipmInfo, *cameraInfo);

    output_tmp=output;

    mcvGetLines(&output_tmp, LINE_VERTICAL, subimageLines, subimageLineScores,
       subimageSplines, subimageSplineScores, stopLineConf,
        state);
...
for(i=0;i<subimageSplines.size();i++)
{        
 CvMat *points = mcvEvalBezierSpline(subimageSplines[i], .1); 
     CvMat* p = mcvExtendPoints(&output_tmp, points,
                stopLineConf->extendIPMAngleThreshold,
                stopLineConf->extendIPMMeanDirAngleThreshold,
                stopLineConf->extendIPMLinePixelsTangent,
                stopLineConf->extendIPMLinePixelsNormal,
                stopLineConf->extendIPMContThreshold,
                stopLineConf->extendIPMDeviationThreshold,
                cvRect(0, stopLineConf->extendIPMRectTop,
                  output_tmp.width-1,
                  stopLineConf->extendIPMRectBottom-stopLineConf->extendIPMRectTop),
                true);
 
        Spline spline = mcvFitBezierSpline(p, stopLineConf->ransacSplineDegree);
        
      //SHOW_IMAGE(&ouyput_tmp, "dont know anything", 1);

  extended_spline.push_back(spline);
}
subimageSplines=extended_spline;*/
//////////////////////////////////////////////////////////////////
    ...
    for (unsigned int k=0; k<subimageSplines.size(); k++)
    {
      ...
      cout<<"This is For Spline number"<<k<<endl; 
      if(k==0)
      {
       for(unsigned int px=0;px<4;px++)
       {
       cout<<cvPointFrom32f(subimageSplines[k].points[px]).x<<" "<<cvPointFrom32f(subimageSplines[k].points[px]).y<<endl;
      SHOW_IMAGE(subimage, "This is my subimage", 1);
      cvCircle( subimage, cvPoint(cvPointFrom32f(subimageSplines[k].points[px]).x,cvPointFrom32f(subimageSplines[k].points[px]).y) , 8, CV_RGB(255, 255, 255),-1 );
        }  
       // track(subimage, subimageSplines);
      }
     // cout<<subimageSplines.size()<<endl;
...
/*  mcvPostprocessLines(image, clrImage, fipm, ipm, *lanes, *lineScores,
  ...
  mcvPostprocessLines(image, clrImage, &output_tmp,&output_tmp, *lanes, *lineScores,
                      *splines, *splineScores,
                      stopLineConf, state, ipmInfo, *cameraInfo); //rawipm
...
// 		      cvSize(image->width, image->height));
  ...
  if (lineConf->checkLaneWidth)
   ...
   cout<<"lineConf->ransac: : " <<lineConf->ransac<<endl;
    if (lineConf->ransac)
    {
       cout<<"okay8"<<endl;
      //do RANSAC lines?
                                ...
                                bool extendSpline)
{
  ...
  if (extendSpline)
  {
    ...
    Line line;
    line.startPoint = cvPoint2D32f(CV_MAT_ELEM(*points, float, p0, 0) - 5 *
                                   CV_MAT_ELEM(*tangents, float, 0, 0),
                                   CV_MAT_ELEM(*points, float, p0, 1) - 5 *
                                   CV_MAT_ELEM(*tangents, float, 0, 1));
    ...
    line.endPoint = cvPoint2D32f(CV_MAT_ELEM(*points, float, p1, 0) + 5 *
                                 CV_MAT_ELEM(*tangents, float, 1, 0),
                                 CV_MAT_ELEM(*points, float, p1, 1) + 5 *
                                 CV_MAT_ELEM(*tangents, float, 1, 1));
...
CvMat* mymcvGetBezierSplinePixels(Spline &spline, float h, CvSize box,
                                bool extendSpline)
{
 int xtndid=3;
  //get the points belonging to the spline
  ...
  /* for(int px=0;px<2;px++)
      {
       avg+=spline.points[px+1].y-spline.points[px].y; 
      } 
      cout<<"avg    : : : :"<<avg<<endl;
    spline.points[3].y=spline.points[2].y+avg/2;*/
      //cout<<cvPointFrom32f(spline.points[px]).x<<" "<<cvPointFrom32f(spline.points[px]).y<<endl;
 ...
 for (int i=0; i<points->height; i++)
 {
  ...
  CvMat *pixelst0, *pixelst1;
    ...
    line.startPoint = cvPoint2D32f(CV_MAT_ELEM(*points, float, p1, 0),
                                   CV_MAT_ELEM(*points, float, p1, 1));
    //intersect the line with the bounding box
    mcvIntersectLineWithBB(&line, cvSize(box.width-100, box.height-100), &line);
    //get line pixels
    pixelst1 = mcvGetLinePixels(line);
    numin += pixelst1->height;
  }
                         ...
                         vector<float> &splineScores, LineState* state)
{
  ...
  cout<<"okay ransac 4"<<endl;
  mcvGroupBoundingBoxes(boxes, lineType, overlapThreshold);
  cout<<"okay ransac 5"<<endl;
...
/*void learn(CvMat *learn_image,CvMat *pixels)
{
      Mat track=Mat(learn_image);
...
//waitKey(0);
  //get spline pixels
 //cout<<endl<<endl<<"These are Spline Pixels before calling"<<endl;
//for(int px=0;px<4;px++)
  //     cout<<cvPointFrom32f(spline.points[px]).x<<" "<<cvPointFrom32f(spline.points[px]).y<<endl;
  //while(y--)
...
for(int px=0;px<4;px++)
       cout<<cvPointFrom32f(spline.points[px]).x<<" "<<cvPointFrom32f(spline.points[px]).y<<endl;
  pixels = mymcvGetBezierSplinePixels(spline, 0.05,
                                           cvSize(image->width, image->height),
                                          false);
for (int i=0; i<=0/*pline.degree+1*/; i++)
    cvCircle(image, cvPointFrom32f(spline.points[i]), 5,  CV_RGB(0,0,255)/*color*/, -1);

/*float sumx, sumy;

for(int i=0;i<4;i++)
{
  sumx+=spline.points[i].x;
  sumy+=spline.points[i].y;
}
float avgx=sumx/4;
float avgy=sumy/4;
float avg_diff=0;
float avg_diff_square=0;
for(int i=0;i<4;i++)
{
  avg_diff=avg_diff+(spline.points[i].x-avgx)*(spline.points[i].y-avgy);
  avg_diff_square=avg_diff_square+(spline.points[i].x-avgx)*(spline.points[i].x-avgx);
}

float m=avg_diff/avg_diff_square;
float b=avgy-(m*avgx);
//float ans_y=m*ans_x+b;


spline.points[0].x=spline.points[1].x+5;
  float ans_x=spline.points[1].x+5;
  spline.points[1].y=m*ans_x+b; */
cv::Mat A(4,2,CV_32FC1);
for(int px=0;px<4;px++)
  {    
    A.at<float>(px,0) = spline.points[px].x;
    A.at<float>(px,1) = 1;
    //A.at<float>(px,2) = 1;   
  }
  cv::Mat inverted(2,4,CV_32FC1);
...
for(int px=0;px<4;px++)
{
  B.at<float>(px,0)= spline.points[px].y;
}
  Mat C =inverted*B;
  
  float m=C.at<float>(0,0);
  float c=C.at<float>(1,0);
  ...
  cout<<"my m are :: "<<m<<endl;
  cout<<"my c are :: "<<c<<endl;

  //spline.points[0].x=spline.points[1].x+5;
  //spline.points[0].y=((-1*a*spline.points[0].x)-1)/b;
  int temp_x, temp_y,prev_temp_x, prev_temp_y;
  prev_temp_y=200;
  prev_temp_x=(prev_temp_y-c)/m;

  for(int temp_y=200;temp_y<350;temp_y++)
  {
    temp_x=(temp_y-c)/m;
    cvLine(image, cvPoint(temp_x,
                          temp_y),
           cvPoint(prev_temp_x, prev_temp_y), color, width);
  prev_temp_x=temp_x;
  prev_temp_y=temp_y;
  }

///
/*if((spline.points[1].x-spline.points[0].x)>300 && (spline.points[1].y-spline.points[0].y)<100)
  spline.points[0].x=spline.points[1].x-200;
else if((spline.points[0].x-spline.points[1].x)>300 && (spline.points[1].y-spline.points[0].y)<100)
  spline.points[0].x=spline.points[1].x+200;
 

 pixels = mcvGetBezierSplinePixels(spline, 0.05,
                                           cvSize(image->width, image->height),
                                          false);*/
...
/*float slope1=(spline.points[2].y-spline.points[3].y)/(spline.points[2].x-spline.points[3].x);
float slope2=(spline.points[1].y-spline.points[2].y)/(spline.points[1].x-spline.points[2].x);
float slope4=(spline.points[0].y-spline.points[1].y)/(spline.points[0].x-spline.points[1].x);
float slope3;
if(slope2>0)
slope3=(slope2+slope1+slope4)/3;//(slope1+slope2)/2;
else
  slope3=slope2;//(slope1+slope2)/2;

//float distance=(spline.points[3].y-spline.points[2].y)*(spline.points[3].y-spline.points[2].y)+((spline.points[3].x-spline.points[2].x)*(spline.points[3].x-spline.points[2].x));
// distance=sqrt(distance);
int dis = 50;


float theta=atan(slope3);
float cosx=cos(theta);//sqrt((slope3*slope3)/(1+slope3*slope3));
float sinx=sin(theta);//sqrt(1-cosx*cosx);
 cout<<"before"<<endl;

for(int px=0;px<4;px++)
       cout<<cvPointFrom32f(spline.points[px]).x<<" "<<cvPointFrom32f(spline.points[px]).y<<endl;
//cout<<" before "<<cvPointFrom32f(spline.points[0]).x<<" "<<cvPointFrom32f(spline.points[0]).y<<endl;
if(sinx<0)
sinx=-1*sinx;
if(cosx<0)
cosx=-1*cosx;
...
cout<<"cosx =="<<cosx<<endl<<"sinx =="<<sinx<<endl;*/

/*if(slope3<0)
{
  if((spline.points[1].y-dis*sinx)<300 && (spline.points[1].y-dis*sinx)>0)
spline.points[0].y=spline.points[1].y-dis*sinx;
else
  spline.points[0].y=spline.points[1].y;

 if((spline.points[1].x+dis*cosx)<1200 && (spline.points[1].x+dis*cosx)>0)
spline.points[0].x=spline.points[1].x+dis*cosx;
else
spline.points[0].x=spline.points[1].x;
...
for(int px=0;px<4;px++)
       cout<<cvPointFrom32f(spline.points[px]).x<<" "<<cvPointFrom32f(spline.points[px]).y<<endl;
//cout<<" after"<<cvPointFrom32f(spline.points[0]).x<<" "<<cvPointFrom32f(spline.points[0]).y<<endl;
...
/*cv::Mat track = cv::Mat::zeros(input.rows,input.cols, CV_8UC1);
        for (int i=0; i<pixels->height-1; i++)
          {
            ...
            for(int i = 0; i < it.count; i++, ++it)
            {
              buf[i] = (const Vec3b)*it;
               points[i] = it.pos();
               //cvCircle( image, cvPoint(points[i].x,points[i].y) , 8, CV_RGB(0, 0, 255),-1 );
                track.at<uchar>(points[i].y,points[i].x)=255;
            }
  ...
  //cout<<endl<<endl<<"These are Spline Pixels after calling"<<endl;
//for(int px=0;px<4;px++)
  //     cout<<cvPointFrom32f(spline.points[px]).x<<" "<<cvPointFrom32f(spline.points[px]).y<<endl;
  //if no pixels
  if (!pixels)
  ...
  /*for (int i=0; i<pixels->height-1; i++)
   ...
   {  if((int)cvGetReal2D(pixels, i, 1)>200)  //     color);
 ...
 for (int i=0; i<spline.degree+1; i++)
   cvCircle(image, cvPointFrom32f(spline.points[i]), 3,  CV_RGB(255, 0, 0)/*color*/, -1);
...
SHOW_IMAGE(extendedPoints, "danish", 10);
  //clear
 //cout<<str<<endl;
  backPoints.clear();
  frontPoints.clear();
